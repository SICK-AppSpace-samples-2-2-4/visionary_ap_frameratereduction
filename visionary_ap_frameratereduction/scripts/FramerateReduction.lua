...
local CONFIG_NTH_FRAME = 5            -- Show one frame per n received frames
local CONFIG_ENABLE_AVERAGING = true  -- Show image as the average of the last n frames
local CONFIG_RANGE_LOW = 1000         -- The range of pixel visualizing, in mm. Pixels
local CONFIG_RANGE_HIGH = 5000        -- outside the range will be either black or white.
...
deco:setRange(CONFIG_RANGE_LOW, CONFIG_RANGE_HIGH)
local viewer = View.create('2DViewer')

-- Create camera provider handle
...
local function displayEveryNthFrame(image)
  ...
  if imagesSinceDisplay == CONFIG_NTH_FRAME then    --check if enough frames have passed
...
local function displayEveryNthFrameWithAveraging(image)
  ...
  -- extracting missing parts of the image.
  ...
  if imagesSinceDisplay == CONFIG_NTH_FRAME then
...
--@main()
local function main()
  -- Registration of different callback function depending if averaging is on. 
  if CONFIG_ENABLE_AVERAGING then
